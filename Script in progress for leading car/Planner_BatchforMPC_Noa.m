function[feas, xIter,uIter, JIter] = Planner_BatchforMPC_Noa(u2d,u1d,qx,rou1,rou2,Tx,M,Q, R, N, x0, xL, xU, uL, uU,obstacle,safetyR)
%% Solve Initial guess
% Define state matrix
x = sdpvar(4,N+1);
...
for i = 1:N
    constraints = [constraints  xL<=x(:,i)<=xU -0.6<=u(i)<=0.6 ];%upper and lower bound of input and state
...
%                 d=P.distance([obstacle(1);obstacle(2)]);
                constraints = [constraints  xL<=x(:,i)<=xU -0.6<=u(i)<=0.6 slack(i)>=0];%upper and lower bound of input and state
                constraints = [constraints  x(:,i+1) == bikeFE_Noa(x(:,i),u(i))];%system dynamics
