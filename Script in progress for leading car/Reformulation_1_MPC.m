function [feas, xOpt, uOpt,JOpt] = Reformulation_1_MPC(x0, M, N,P0, R, xL, xU, uL, uU,obstacle,safetyR)
%carry out MPC on a simulation horizon of M
feas=zeros(1,M);
xOpt=zeros(size(P0,2)-1,M+1);xOpt(:,1)=x0;
uOpt=zeros(size(R,2)-1,M);
JOpt=zeros(1,M);
pred=zeros(size(P0,2)-1,N+1,M+1);
K=1;
for i = 1:M
[feas(i), xIter,uIter, JIter] = Reformulation_1_BatchMPC(P0,R, N, x0, xL, xU, uL, uU,obstacle,safetyR);
if feas(i)==false
    for t= i+1:M
        feas(t)=false;
    end
