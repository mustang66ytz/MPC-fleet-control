function[feas, xIter,uIter, JIter] = Planner_Batch_shape(Q, R, N, x0,xN, xL, xU, uL, uU,obstacle,safetyR)
%% Solve Initial guess
% Define state matrix
x = sdpvar(4,N+1);
slack =sdpvar(1,N+1);
...
for i = 1:N
    ...
    constraints = [constraints  x(:,i+1) == TruckFE(x(:,i),u(:,i))];%system dynamics
    if i <= N-1       
